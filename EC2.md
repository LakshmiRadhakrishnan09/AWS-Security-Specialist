...
* Hadoop File System. Mongo db uses Instance Store. For availability, data is replicated between EC2 clusters.
...
* Snapshots stored in S3. Can be used to replicate in another AZ.( Disaster Recovery)
...
Data leek issue in 2019. AWS SSM having all GET and PUT request on all buckets. AWS deprectade this role. The AmazonEC2RoleforSSM policy should be replaced with **AmazonSSMManagedInstanceCore**. Do dont use that role. Also new version of IMDS is released. Prevent Server Side Request Forgery attack that depends on request forwarding. Using session window. 

**Token is issued only if X-forwareded-For is not present.** When a firewall fwds the request it will have x-forwareded-for, so token cannot be generated by outside parties(other than ec2).Also ttl of 1 limit number of hops(only to EC2).

Session Token need to be passed in subsequent requests as **x-aws-ec2-meta-data-token:<>.** A PUT request is used for establishing a session with IMDS version 2. In response, IMDS issues a session token that is valid for TTL specified in the PUT call. With IMDS v2, all calls to metadata service(get role) must include the token in the X-aws-ec2-metadata-token header. PUT is used , because several firewalls, and proxies do not forward PUTs. Besides, IMDS V2 does not issue a token when the PUT request has an X-Forwarded-For header.

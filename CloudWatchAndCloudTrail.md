...
- Configure Agent to use instance IAM role
...
Any change on state is send as events to EventBridge. Subscriber services like EC2, Lambda can subscribe to these events.
...
CloudTrail Evenst are captured in Event History, S3, CloudWatch Logs, CloudWatch Events, CloudTrail Insights.
...
##### CloudTrail Event History : 
 ...
 - Not very useful as we need to check events in different regions( Console Login in N.Virginia, S3 calls and VPC calls in specific regions)
...
When u create a trail for all regions, u specify single Log Group, so u can view all logs in Log Insights.
...
**How to detect and take action if root identity is used? Configure Rule in event bus for Console Sign In by Root and subscribe SNS topic. Or configure CloudWatch Metric for Log Group. Create Metric Filter** \
...
##### To Use EventBridge to react to API calls made by an Amazon service that are recorded by Amazon CloudTrail.

1. Create trial
2. Create a lambda function to respond to event in eventBridge
3. Create a rule in EventBridge. For Rule type, choose Rule with an event pattern.
          3.1. For Event bus, choose the event bus that you want to associate with this rule. If you want this rule to match events that come from your account, select **default.** When an Amazon service in your account emits an event, it always goes to your account’s default event bus.
          3.2. For Event source, choose Amazon services.
          3.3. For Event pattern, do the following:
                  For Event source, select EC2 from the drop-down list.
                  For Event type, select **Amazon API Call via CloudTrail** from the drop-down list.
                  Choose Specific operation(s) and enter **StopInstances**.

##### To react to EC2 state change

1. No trail required
2. Create rule in Event Bridge. For Rule type, choose Rule with an event pattern.
...
##### To react to S3 object level operation

1. Enable trail for data events for the S3 bucket/object
2. Create rule in EventBridge. For Rule type, choose Rule with an event pattern.
          For Event pattern, do the following:
...
##### Schedule automated Amazon EBS snapshots using EventBridge

1. For Rule type, choose Schedule.
2. For Schedule pattern, choose A schedule that runs at a regular rate, such as every 10 minutes. and enter 5 and choose Minutes from the drop-down list.

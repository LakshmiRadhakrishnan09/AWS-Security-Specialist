...
* Condition
  ...
  * aws:SecureTransport
  * aws:sourceIP: Only public IP addresses or public IP ranges are supported.
  * aws:sourceVpc
  * aws:sourceVpce
  * ec2:ResourceTag
...
External ID is a random string **generated by the third party**. Third party(example.com) need to generate unique External Id for each AWS account they service. Example.com share account Id and externalId to u. In ur account, create a role and with trust policy. You share the Amazon Resource Name (ARN) of the role to Example Corp.
...
- Using AWS IAM Identity Center : for single centralized directory(Single AD)
- Using IAM: If you use multiple directories, or want to manage the permissions based on user attributes, consider AWS IAM as your design alternative.

**AWS Organisation enables SSO**

#### IAM vs IAM Identity Center

IAM: manage access to AWS services and resources within a sinhle AWS account 

IAM Identity Center : manages access **for all AWS accounts within an AWS Organization**, as well as access to other cloud applications, e.g., Salesforce.

IAM: supports setting up multiple identity providers per account. Using AWS IAM, you need to log in to each AWS account and configure the identity provider. 

AWS IAM Identity Center **only supports a single identity provider.** Using AWS IAM Identity Center, you configures thr identity provider once, and it becomes the identity provider for all new and existing AWS accounts. 

IAm Identity Center: Can reuse policies and permission for all accounts.
...
AWS Application : Service Provider
...
Parties involved in federation: AD(identity provider) and sts(relying party) 
...
* Simple AD: Standalone directory. No trust establishment.Not compatible for RDS-MySQL. Small : 500 users. Large : 5000 users
...
``
 ...
 Principal elemnt : used in resource policies and in trust policies. you’ll get errors if you try to use the Principal element in an IAM Role policy
...
Summary:
If resource based policy has arn of user: Intersection of identity policy , session policy , permission boundary and resource policy. ( Allow permissions that are allowed by all these policies).
If resource bases policy has arn of session: Intersection of identity policy , permission boundary and session policy. Then allow all permissions from resource based policy.
...
You can't recover an access key after you delete it. Any users or applications that are using the access key won't be able to programmatically access your account and resources after you delete the key. Be sure to check that the access key is no longer in use before you delete it. 
 ...
 You cannot use groups in resource based policy or as principal in any policy.
 ...
 IAM user password(not-root) : ask your administrator to reset your password. \
 IAM user access keys : need new access keys. 

 SCP : Deny access to AWS based on the requested AWS Region

 Deny if "aws:RequestedRegion" is not from eu-west-1. But inorder to allow global services u need to add global service to exclusion list.  IAM, CF, GA should be excluded using "NotAction". Global applications use KMS and ACM with global end point. So add that also(even if KMS dn ACM have regional endpoints).
